#!/bin/bash
# Wartungs-Update-Zyklus fuer den USV-NUT-Pi mit aktivem OverlayFS.
#
...
#
# Nutzung:
#   sudo usv-pi-update.sh status   Aktuellen Zustand anzeigen
#   sudo usv-pi-update.sh begin    Phase 1: OverlayFS deaktivieren
#                                  -> danach manuell rebooten
#   sudo usv-pi-update.sh apply    Phase 2 (nach dem Reboot): Boot-RO
...
# genau das ist real aufgetreten (verifiziert 2026-08-15). /boot/firmware
# wird vom Overlay nie erfasst (overlayroot behandelt vfat als
# "fs-unsupported"), bleibt also ueber den Reboot hinweg garantiert
# erhalten.
STATE_FILE="$BOOT_DIR/.usv-pi-update.state"
...
# Schreibt die State-Datei auf der Boot-Partition, remountet sie dafuer
# bei Bedarf kurz beschreibbar (unabhaengig vom OverlayFS-Zustand, da
# /boot/firmware live remountbar ist).
write_state() {
  local was_ro=0
...
print_status() {
  ...
  if systemctl is-active --quiet nut-server || systemctl is-active --quiet nut-monitor; then
    echo "Hinweis: nut-server/nut-monitor laufen aktiv - waehrend der beiden Reboots ist die USV-Ueberwachung kurz unterbrochen."
  fi
}
...
cmd_begin() {
  ...
  if [ "$(overlay_now)" -eq 1 ]; then
    # War schon vor diesem Aufruf inaktiv (z.B. noch nie eingerichtet) -
    # kein Zwischenreboot noetig, direkt mit 'apply' weitermachen.
